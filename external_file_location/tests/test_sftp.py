...
class TestNewSource(common.TransactionCase):
    def setUp(self):
        ...
        self.location = self.env['external.file.location'].create(loc_vals)
        task_vals = {
        ...
        self.task = self.env['external.file.task'].create(task_vals)
    ...
    def test_00_sftp_import(self):
        with server_mock(
            ...
            self.task.with_context(test=True).run_import()
