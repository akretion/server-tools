...
class AbstractFSTask(object):
    ...
    def __init__(self, env, config):
        ...
        self.attachment = config.get('attachment', False)
    ...
    def _process_file(self, conn, file_to_process):
        ...
        data = self._handle_new_source(
            conn,
            self.path,
            self.file_name,
            self.move_path)
        attachment_vals = self.task._prepare_attachment_vals(self, data)
        attachement = self.env['ir.attachment.metadata'].create(
                attachment_vals)
        move = False
        rename = False
    ...
    def _upload_file(self, conn, filename, filedata):
        upload_directory = self.path or '.'
