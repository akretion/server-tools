...
class StorageBackend(models.Model):
    ...
    import_pattimpex_task_count = fields.Integer(
        "Import Tasks", compute="_compute_import_pattimpex_task_count"
    )
    export_pattimpex_task_count = fields.Integer(
        "Export Tasks", compute="_compute_export_pattimpex_task_count"
    )

    def _compute_import_pattimpex_task_count(self):
        for rec in self:
            rec.import_pattimpex_task_count = len(
                rec.synchronize_task_ids.filtered(lambda t: t.method_type == "import_pattimpex")
            )

    def _compute_export_pattimpex_task_count(self):
        for rec in self:
            rec.export_pattimpex_task_count = len(
                rec.synchronize_task_ids.filtered(lambda t: t.method_type == "export_pattimpex")
            )
